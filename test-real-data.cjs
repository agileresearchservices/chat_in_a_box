...
async function runTests() {
  ...
  for (const query of testQueries) {
    ...
    // Try to query the agent directly
    try {
      ...
      // We expect stores to be found for these real cities/states/ZIPs
      if (
        query.includes('Port Eric') || 
        query.includes('South Bruce') || 
        query.includes('Thomas') || 
        query.includes('East David') || 
        query.includes('Parkville') || 
        query.includes('Johnport') ||
        query.includes(' FL') || 
        query.includes(' CA') || 
        query.includes(' NY') || 
        query.includes('420') || 
